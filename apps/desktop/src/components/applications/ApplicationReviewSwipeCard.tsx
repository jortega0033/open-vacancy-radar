 ...
 */
export function ApplicationReviewSwipeCard({ attempt, snapshot, screenshotBase64, busy, onApprove, onSkip }: ApplicationReviewSwipeCardProps) {
  const [dragX, setDragX] = useState(0);
  const dragOriginRef = useRef<number | null>(null);
  const [dragging, setDragging] = useState(false);

  const fieldCount = snapshot.fields.length;
  const requiredCount = snapshot.fields.filter((field) => field.required).length;
      ...
      >
        ...
        <div className="border-b border-base-300 px-5 py-3.5">
          ...
          <p className="text-xs text-base-content/60">
            {fieldCount} field{fieldCount === 1 ? '' : 's'} filled ({requiredCount} required)
          </p>
        </div>

        <div className="bg-base-200">
          {/* A live screenshot of the application page exactly as this review opened it -- not a
           * re-description of it. Note what this is NOT: this app has no wired path yet that
           * tailors a CV/cover letter and applies the resulting field map to the page before a
           * review opens (packages/client's runFieldMapGeneration has no production caller today),
           * so today this screenshot shows the page's fields in whatever state they were actually
           * in when this review's snapshot was taken -- filled only if something already filled
           * them earlier in this same browser session. Confirm the fields below actually show the
           * answers you expect before submitting; don't take "a screenshot exists" as proof of that. */}
          <img
