...
function seedPre0005MigrationsFolder(root: string): string {
  ...
  expect(journal.entries.at(-1)?.tag).toBe('0005_lean_echo');
...
describe('migration 0005 changes the default without rewriting existing data', () => {
  ...
  it('gives a genuinely new install (no prior settings row) the new, bias-free default', () => {
    ...
    // this exercises the migration's DEFAULT clause itself, not a copied value.
    const { db, close } = createWorkspaceDb(dir);
    try {
      const settings = workspace.getSettings(db);
      expect(settings.sponsorOnlyDefault).toBe(false);
      expect(settings.indVerificationEnabled).toBe(false);
