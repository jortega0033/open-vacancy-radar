...
// preload.ts exposes each bridge through its own named `exposeInMainWorld` call, so loading one
// namespace does not replace another as a shared `exposedApi` variable would.
...
describe('electron/preload.ts: workspace bridge', () => {
  ...
  it('exposes no generic IPC passthrough or channel argument anywhere in the namespace', async () => {
    const api = await loadPreload('workspace');
    expect(api.invoke).toBeUndefined();
    expect(api.send).toBeUndefined();
    expect(api.ipcRenderer).toBeUndefined();
