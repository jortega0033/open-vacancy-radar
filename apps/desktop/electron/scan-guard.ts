 ...
 * thing that stops a scan started by another process. One example is `pnpm vacancies:scan` pointed
 * at the same userData database. This closes the gap flagged in the earlier review, where the
 * desktop app's global-remote handler used only an in-process Boolean.
 *
...
export function createScanGuard(getLock: () => ScanLock | undefined) {
  ...
  return async function runExclusiveScan<T>(
    ...
    try {
      ...
      // Cleared however the scan ended. A failed scan must not leave the app in a state where
