 ...
 * runtime dependency, so it stays testable. The IPC handler only chooses a path and calls this
 * function.
 *
 * PDF extraction uses `unpdf`, a pure-JavaScript wrapper around a serverless build of Mozilla's
 * pdf.js. It adds no runtime dependencies. It was chosen over `pdfjs-dist`'s legacy Node build
 * because it needs no worker file to locate, no `standardFontDataUrl`/`isEvalSupported` tuning,
 * and no `@napi-rs/canvas` (a native, prebuild-install-backed optional dependency of pdfjs-dist)
 * for the text-only path. The text-only path therefore needs no native rebuild toolchain in
 * Electron's main process. `pdf-parse` was rejected: it is effectively unmaintained and its entry
 * point runs a debug harness that reads a fixture file from disk when imported without a module
 * parent.
 */
...
/** Collapses the irregular whitespace pdf.js produces so the prompt stays readable and compact. */
...
async function extractPdfText(bytes: Uint8Array): Promise<string> {
  // Imported only when needed, so the larger pdf.js build loads after the user picks a PDF and
  // importing this module in a test does not load pdf.js.
 ...
 * on every platform (typing a name or choosing "All files" on some window managers). Validation
 * happens on both sides of the boundary, not only in the picker.
 */
export async function readCvFile(filePath: string): Promise<CvFileContent> {
  ...
  // afterward would still spare pdf.js, but not the read itself. A large file could have
  ...
  if (!text) {
    throw new Error(
      extension === 'pdf'
        ? `No selectable text found in "${fileName}". It may be a scanned image; export a text-based PDF or paste the CV as .txt.`
        : `"${fileName}" is empty.`,
    );
  }
