...
test.describe('Search', () => {
  test('shows the no-report-yet empty state for a fresh workspace, per market', async ({ window }) => {
    ...
    // Fresh workspace: neither market has ever been scanned, so SearchPage's hydrate-then-optionally
    // -scan effect resolves to `hasReport === false` and renders the "No search yet" EmptyState
    // (SearchPage.tsx line ~397), never the results list.
    ...
    // Netherlands-only filter chips must disappear ...
    ...
    // Switching back to Netherlands restores its own copy and its own filter chips.
