...
All design tokens live in **`src/styles/tokens.css`**, which is the app's only stylesheet entry
...
- **`success`, `warning` and `error` use green, amber, and red**, with light and dark variants.
  The app uses them for employer verification, scan results, application status, and runtime
  status. Those are the only state hues. `info` remains grayscale because it describes progress,
  not an outcome.
...
- **Flat.** `--depth` and `--noise` are `0`. Radii use the prototype's 4–8px range
  (`--radius-field` `0.375rem`, `--radius-box` `0.5rem`).
- **Typography.** Native sans stack via `font-sans`, monospace via `font-mono`, and a small
  type scale (`text-xs` … `text-2xl`). Do not use sizes above `text-2xl`.
...
Density is a plain custom property rather than a theme because it is independent of color;
...
`rounded-[7px]`) in component JSX. If a new value is required, add it as a token or a
`@utility` in `tokens.css` first, then reference it.

Hue is allowed only through the documented state tokens. Do not add one-off colors at component
call sites.
...
This rule keeps future UI edits consistent: change a token once in
