...
describe('consumeSmokeStream', () => {
  it('succeeds with normalized content on exactly one terminal event', async () => {
    const outcome = await consumeSmokeStream(iterableFrom([{ type: 'assistant.message', text: 'hello' }, { type: 'session.completed' }]), {
      timeoutMs: 5_000,
    });
  ...
  it('succeeds but reports no content when the terminal event carries no normalized text', async () => {
    const outcome = await consumeSmokeStream(iterableFrom([{ type: 'session.completed' }]), { timeoutMs: 5_000 });
    expect(outcome).toEqual({ outcome: 'success', terminalType: 'session.completed', hasContent: false });
  });

  it('thinking.delta is not counted as content -- only assistant.message is', async () => {
    const outcome = await consumeSmokeStream(iterableFrom([{ type: 'thinking.delta', text: 'reasoning...' }, { type: 'session.completed' }]), {
      timeoutMs: 5_000,
    });
    expect(outcome).toEqual({ outcome: 'success', terminalType: 'session.completed', hasContent: false });
  });

  it('cancellation also counts as a valid single completion', async () => {
    const outcome = await consumeSmokeStream(iterableFrom([{ type: 'session.cancelled' }]), { timeoutMs: 5_000 });
  ...
  it('flags a duplicate terminal event as a protocol violation', async () => {
    const outcome = await consumeSmokeStream(iterableFrom([{ type: 'session.completed' }, { type: 'session.completed' }]), { timeoutMs: 5_000 });
  ...
  it('flags an event with no recognizable type as a malformed-stream protocol violation', async () => {
    const outcome = await consumeSmokeStream(iterableFrom([{ notAnEvent: true }]), { timeoutMs: 5_000 });
    expect(outcome.outcome).toBe('protocol_violation');
  ...
  it('flags a stream that ends without ever reaching a terminal event', async () => {
    const outcome = await consumeSmokeStream(iterableFrom([{ type: 'assistant.message', text: 'x' }]), { timeoutMs: 5_000 });
    expect(outcome.outcome).toBe('protocol_violation');
