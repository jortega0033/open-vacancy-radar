...
/**
 * Where SessionManager keeps `AgentSession` records. The interface is synchronous because the only
 ...
 * buffered event history is separate runtime-only state in SessionManager. See
...
/**
 * This is the only `SessionStore` implementation in this milestone and the daemon's default.
 * Sessions remain in memory and do not survive a daemon restart. Swapping in a persistent
