...
This document describes each repository layer, its responsibilities, and its dependencies. See
[protocol-v1.md](protocol-v1.md) for the wire format, [client-sdk.md](client-sdk.md) for client
decisions, and [packaging.md](packaging.md) for packaging details.

## Component diagram

```
┌─────────────────────────┐
│   Renderer (React)        │   window.agentDock.*: seven IPC capabilities only.
...
└─────────────┬────────────┘
              ...
              ▼
┌─────────────────────────┐
│   AgentDockClient           │   Typed daemon SDK: HTTP+SSE, bearer auth, and protocol
│   (packages/client)        │   version compatibility checks. No Electron dependency;
...
app use the same definitions. `packages/client` depends on `packages/shared` only, **never** on
`agent-runtime` or `apps/daemon`, so the graph stays acyclic.
...
|---|---|---|
| A type, Zod schema, or the protocol version | `packages/shared` | anywhere downstream; every other package imports these definitions |
| Provider process spawning, CLI-native parsing, an `AgentEvent` normalization rule | `packages/agent-runtime` | `apps/daemon`; the daemon never parses a provider's raw output |
...
| Anything the daemon exposes to a caller (HTTP/SSE handling, error typing) | `packages/client` | `apps/desktop/electron/main.ts`; main calls `AgentDockClient` methods instead of creating daemon requests |
...
The system has three relevant trust boundaries:

1. **The public internet / an arbitrary webpage → the daemon.** See
   [SECURITY.md](../SECURITY.md). The daemon binds
...
2. **The renderer → Electron main.** The renderer is this repo's own code, not adversarial, but
   ...
   functions exposed by the preload bridge.
...
The project does **not** defend against another process running as the same OS user. See
[SECURITY.md](../SECURITY.md#what-this-does-not-claim-to-protect-against).
...
1. **A malicious webpage should never be able to run a coding agent on your machine.** Keeping
   ...
   defined interface that can be tested independently of the renderer.
...
5. The daemon validates the body again with Zod. Client-side validation is separate (see
   [client-sdk.md](client-sdk.md)). It checks that the provider exists and that the `cwd`
...
11. The CV and letter features consume the renderer's `onSessionEvent` callback through
    `components/cv/useAgentRun.ts`, which switches on `event.type` and never on provider ID.
...
`SessionManager`. **Persistence is out of scope.** See
...
[providers.md#provider-capabilities](providers.md#provider-capabilities) for the
`ProviderCapabilities` shape and the fields supported by Claude and Codex. The
...
- resolves the executable via `findExecutable` (PATH lookup plus a fixed fallback directory list;
...
This is not the only way to run it: `pnpm daemon` starts the same server
...
## Current omissions (v0.2)

The current release omits the following features. They are also listed as out of scope in
...
- **One daemon per app id, enforced** (see [Project identity](#project-identity) and
  ...
  products built on AgentDock coexist by using different app ids, but two instances of the
...
- **No API-key/cloud provider mode.** Everything here assumes a locally authenticated CLI. A second
  authentication model requires separate API and credential handling.
- **No auto-update, telemetry, or crash reporting.** These features require additional network,
  privacy, and update-security decisions.
...
AgentDock provides a reusable local runtime and typed interfaces for products built in this
workspace. It is intended to be copied and adapted, not installed as a set of independent npm
packages. All six workspace packages are `private: true`. The reusable library packages point
`main` and `types` at their TypeScript source for workspace development. The desktop app instead
points `main` at `dist-electron/main.js`, and the daemon is an internal application without a
public `types` entry. None is configured for installation from npm outside this workspace.

The internal boundaries still apply. `packages/shared`'s types and Zod schemas,
`AGENT_DOCK_PROTOCOL_VERSION`, and `@agent-dock/client`'s typed interface keep the daemon, client,
and desktop app consistent inside this repository and derived projects. A wire-format change must
go through one shared definition instead of being
...
External npm publication can be considered later. See
[client-sdk.md](client-sdk.md#using-it-from-a-workspacefork-not-from-outside-the-repo) for
...
See [DEVELOPMENT.md](../DEVELOPMENT.md) for setup and file-level guidance for common changes.
