...
const active = (
  ...
  provider,
  adapter: 'active',
  reason: 'Enabled source integration using a public API, structured endpoint, RSS feed, or MCP endpoint.',
});
...
export function globalRemoteSourceRegistry(config: GlobalRemoteConfig): SourceRegistryEntry[] {
  ...
  return [
    ...
    reed,
    jobspipe,
    entry({ id: 'careeronestop', name: 'CareerOneStop Jobs API', url: 'https://api.careeronestop.org/api-explorer/home/index/JobSearchV2_GetJobsByKeywordAndOnetCode', transport: 'api', state: 'configuration_required', provider: null, reason: 'Requires an approved user ID and bearer token; no adapter will be implemented until the live API contract can be verified.' }),
    ...
    entry({ id: 'ziprecruiter', name: 'ZipRecruiter MCP', url: 'https://api.ziprecruiter.com/mcp/docs', transport: 'mcp', state: 'blocked', provider: null, reason: 'Direct requests currently receive HTTP 403 from Cloudflare; recorded without bypass attempts.' }),
    ...
    entry({ id: 'wellfound', name: 'Wellfound', url: 'https://wellfound.com/jobs', transport: 'none', state: 'manual_only', provider: null, reason: 'No verified public job-search API or feed supports reliable automated retrieval.' }),
    entry({ id: 'welcome_to_the_jungle', name: 'Welcome to the Jungle', url: 'https://www.welcometothejungle.com/en/jobs', transport: 'none', state: 'manual_only', provider: null, reason: 'No verified public job-search API or feed supports reliable automated retrieval.' }),
    entry({ id: 'built_in', name: 'Built In', url: 'https://builtin.com/jobs', transport: 'none', state: 'manual_only', provider: null, reason: 'No verified public job-search API or feed supports reliable automated retrieval.' }),
    ...
    entry({ id: 'monster', name: 'Monster', url: 'https://www.monster.com/jobs/', transport: 'none', state: 'manual_only', provider: null, reason: 'No verified public job-search API or feed supports reliable automated retrieval.' }),
    entry({ id: 'simplyhired', name: 'SimplyHired', url: 'https://www.simplyhired.com/search', transport: 'none', state: 'manual_only', provider: null, reason: 'No verified public job-search API or feed supports reliable automated retrieval.' }),
    entry({ id: 'remote_co', name: 'Remote.co', url: 'https://remote.co/remote-jobs/developer/', transport: 'none', state: 'manual_only', provider: null, reason: 'No verified public API or feed; the HTML-only portal is not collected automatically.' }),
    entry({ id: 'nodesk', name: 'NoDesk', url: 'https://nodesk.co/remote-jobs/developer/', transport: 'none', state: 'manual_only', provider: null, reason: 'No verified public API or feed; the HTML-only portal is not collected automatically.' }),
    entry({ id: 'justremote', name: 'JustRemote', url: 'https://justremote.co/remote-developer-jobs', transport: 'none', state: 'manual_only', provider: null, reason: 'No verified public API or feed; the HTML-only portal is not collected automatically.' }),
    entry({ id: 'dailyremote', name: 'DailyRemote', url: 'https://dailyremote.com/remote-software-development-jobs', transport: 'none', state: 'manual_only', provider: null, reason: 'No verified public API or feed; the HTML-only portal is not collected automatically.' }),
