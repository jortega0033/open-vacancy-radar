...
<title>IND Job Radar — ${escapeHtml(formatDate(report.generatedAt))}</title>
<style>
...
<p class="eyebrow">Candidate ${escapeHtml(report.candidateProfileVersion)} · Scoring ${escapeHtml(report.deterministicScoringVersion)} · Run ${escapeHtml(report.scanStatus)}</p><h1>IND Job Radar</h1>
