...
/**
 * Strict newline-delimited JSON-RPC peer for the headerless Codex app-server protocol. Ported
 * close to verbatim from upstream AgentDock -- this file has no OVR-specific coupling (it only
 * depends on `app-server-support.ts`'s method allowlists, which this repo defines with its own
 * narrower set, and `@agent-dock/shared`'s `validateJsonBounds`, which already exists here
 * unchanged). Every outgoing call is checked against the allowlist before a byte is written, and
 * every incoming frame is checked before it's acted on -- an unlisted method is a protocol
 * violation, not something this transport silently forwards.
 */
export class CodexAppServerRpc {
  ...
  acceptStdout(chunk: Buffer): void {
    ...
    while (offset < chunk.byteLength) {
      ...
      if (this.partial.byteLength + segment.byteLength > MAX_FRAME_BYTES) {
        this.fail(new CodexAppServerProtocolError('frame_too_large', 'Codex app-server frame exceeded 1 MiB'));
        return;
      }
      if (segment.byteLength > 0) {
        this.partial = this.partial.byteLength === 0 ? Buffer.from(segment) : Buffer.concat([this.partial, segment]);
      }
      if (newline === -1) return;
  ...
  endStdout(): void {
    if (!this.fatal && this.partial.byteLength > 0) {
      this.fail(new CodexAppServerProtocolError('frame_invalid', 'Codex app-server ended with an incomplete JSON line'));
    }
  }
  ...
  private handleFrame(frame: Buffer): void {
    ...
    try {
      ...
      this.fail(new CodexAppServerProtocolError('frame_invalid', 'Codex app-server emitted invalid UTF-8 JSON'));
      return;
    }
    ...
    if (boundsIssue) {
      this.fail(new CodexAppServerProtocolError('frame_invalid', 'App-server JSON exceeded structural bounds'));
      return;
    }
    ...
    if (hasMethod) {
      if (hasResult || hasError) {
        this.fail(new CodexAppServerProtocolError('frame_invalid', 'Malformed RPC method envelope'));
        return;
      }
      ...
      if (hasId) this.handleIncomingRequest(value);
        ...
        if (!isCodexAppServerIncomingNotificationMethod(value.method)) {
          this.fail(new CodexAppServerProtocolError('forbidden_method', `Unsupported Codex server notification: ${value.method}`));
          return;
        }
        try {
          this.options.onNotification(value.method, value.params);
        } catch (error) {
          this.fail(error instanceof Error ? error : new CodexAppServerProtocolError('frame_invalid', 'Notification handling failed'));
        }
      }
      return;
    }
  ...
  private handleResponse(value: JsonObject): void {
    ...
    if (!pending) {
      ...
      this.fail(new CodexAppServerProtocolError('response_invalid', `Codex app-server returned a ${qualifier} response id`));
      return;
    }
    this.pending.delete(key);
    this.rememberCompleted(key);
    if (Object.hasOwn(value, 'error')) {
      const errorCode = isObject(value.error) && typeof value.error.code === 'number' ? value.error.code : 'unknown';
      pending.result.reject(new CodexAppServerProtocolError('response_invalid', `Codex app-server rejected ${pending.method} (${errorCode})`));
      return;
    }
  ...
  private handleIncomingRequest(value: JsonObject): void {
    ...
    if (!isCodexAppServerIncomingMethod(value.method)) {
      this.fail(new CodexAppServerProtocolError('forbidden_method', `Unsupported Codex server request: ${value.method}`));
      return;
    }
    ...
    const respond = async (payload: JsonObject): Promise<void> => {
      if (responded || !this.incomingRequestIds.delete(key)) {
        throw new CodexAppServerProtocolError('response_invalid', 'Server request already resolved');
      }
      responded = true;
    ...
    try {
      ...
      this.fail(error instanceof Error ? error : new CodexAppServerProtocolError('interaction_invalid', 'Server request failed'));
      return;
    }
  ...
  private assertOutgoing(method: string): void {
    if (!isCodexAppServerOutgoingMethod(method)) {
      throw new CodexAppServerProtocolError('forbidden_method', `Codex app-server method is not allowlisted: ${method}`);
    }
  }
