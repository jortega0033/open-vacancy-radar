 ...
 * `closeWith()`, which bypasses the cap because those one or two events must still reach
...
export class AsyncChannel<T> {
  ...
  /**
   * Returns `false` if the item was dropped because the channel is already closed or full. This is
   ...
   * channel has already hit its normal limit. Those events are producer-emitted, not attacker-supplied
