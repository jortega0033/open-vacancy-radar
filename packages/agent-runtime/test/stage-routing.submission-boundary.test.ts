...
import * as stageRouting from '../src/policy/stage-routing/index.js';
...
const stageRoutingDir = join(here, '..', 'src', 'policy', 'stage-routing');
 ...
 *
 * The stage router used to live in its own package, `@agent-dock/vacancy-agent-adapter`, and this
 * suite's dependency-graph check was scoped to that package alone. It was folded into
 * `packages/agent-runtime/src/policy/stage-routing` (a DX-audit-ordered package-boundary
 * consolidation, no behavior change) once it had no subsystem of its own left to front, so the
 * checks below are scoped to `agent-runtime` as a whole where the boundary is now a package
 * boundary, and to `src/policy/stage-routing` specifically where it is a directory the router's own
 * files live in.
 */
describe('the stage router cannot authorize or trigger a submission (acceptance check 6)', () => {
  it('has no dependency on the application executor, and the executor has none on it', () => {
    const runtime = packageJson('packages/agent-runtime');
    const executor = packageJson('packages/application-executor');
    const runtimeDeps = { ...runtime.dependencies, ...runtime.devDependencies };
    const executorDeps = { ...executor.dependencies, ...executor.devDependencies };
    expect(Object.keys(runtimeDeps)).not.toContain('@agent-dock/application-executor');
    expect(Object.keys(executorDeps)).not.toContain('@agent-dock/agent-runtime');
  });

  it('imports nothing from the application executor', () => {
