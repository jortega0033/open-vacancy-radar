...
describe('providerCapabilitiesSchema', () => {
  ...
  // AD-15: every known key is optional. An absent key means unsupported, like `false`, so a
...
describe('agentEventEnvelopeSchema', () => {
  ...
  // AD-14: assistant.delta was removed from protocol v1 before any adapter emitted it. See
  // packages/shared/src/events.ts. This pins the removal so it can't silently come back.
  it('rejects assistant.delta because it is not part of protocol v1 (AD-14)', () => {
