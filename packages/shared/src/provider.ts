...
/**
 * This is a string union with no boolean member (AD-13). The previous
 * `boolean | 'unknown'` shape let a lazy `if (status.authenticated)` silently treat "couldn't
 * determine" as "authenticated", which is unsafe for an authentication signal.
...
/**
 * What an AgentDock adapter implements for a provider. This does not describe every feature of the
 ...
 *
 * Every known key is optional (AD-15). **Absent means unsupported**, like `false`, so
